...
try {
    ...
    printStatus("Failed to compare code coverage.", STATUS_ERROR);
}

if ($inputFileOneCoverage < $inputFileTwoCoverage) {
    printStatus(
        'Total code coverage is ' . $inputFileOneCoverage . '% which is below the accepted code coverage ' . $inputFileTwoCoverage . '%',
        STATUS_ERROR
    );
}
